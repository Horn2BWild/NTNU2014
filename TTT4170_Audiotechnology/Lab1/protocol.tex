...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=12cm,keepaspectratio=true]{ir}
...
For measuring the reverbation time of the room the microphone was placed at three different positions. One was chosen in the back of the room, one in the middle and one in a corner in the front. The exact positions in the lecture room can be found in table \ref{tab:revmeasure}. Microphone positions are marked as black dot. The measurements were done with a swept sine with starting frequency 50Hz to an end frequency of 20kHz. This is done in every measurement point, then the results are averaged.
\begin{table}
\begin{center}
\begin{tabular}{|c c|}
\hline
...
\paragraph{$f=2kHz$}
$$\bar{\alpha_{2kHz}}=1-e^{\frac{A-4\cdot m\cdot V}{S}}=0.199$$
$$\alpha_{chair,2kHz}=\frac{\bar{\alpha_{2kHz}}\cdot S-\alpha\cdot (S-S_{chair})}{S_{chair}}=\frac{0.199\cdot 615m^2-0.1\cdot 483m^2}{132m^2}$$
$$\alpha_{chair,2kHz}=0.563$$
\paragraph{$f=4kHz$}
$$\bar{\alpha_{4kHz}}=1-e^{\frac{A-4\cdot m\cdot V}{S}}=0.109$$
$$\alpha_{chair,4kHz}=\frac{\bar{\alpha_{4kHz}}\cdot S-\alpha\cdot (S-S_{chair})}{S_{chair}}=\frac{0.109\cdot 615m^2-0.1\cdot 483m^2}{132m^2}$$
$$\alpha_{chair,4kHz}=0.507$$
