...
\subsection{Parallel FST}
	We parallelized FST using OpenMP. In section \ref{sub:fst} we show the code apply the FST function on each column of $b$. The Fortran code needs to be given a preallocated buffer so we also declare a thread-private buffer $z$ that can be reused each time we call FST or inverse FST in an OpenMP parallel for loop without having to worry about two threads reusing the same buffer.
...
\begin{lstlisting}[caption=Parallelizing FST function]{FST}
int nn = 4*n;
static Real *z;
